 ...
 */
   ...
   */
        ...
        /**
   * Inserts a node in the graph, but it is MY version
   * @param {string} n     The node.
   * @return {Graph} Returns itself.
   */
          
        function addNode2(n){
            if(self.nodesIndex[n.id]){
                return; 
            }
            self.nodes.push(n);
            self.nodesIndex[n.id]=n;
            return self
        };
   ...
   */
        ...
        /**
   * Inserts an edge in the graph, but it is MY version \m|
   * @param {string} e     The edge.
   * @return {Graph} Returns itself.
   */
   
        function addEdge2(e){
            if(self.edgesIndex[e.id.toString()])throw Error('Edge "'+e.id.toString()+'" already exists.');
            if(!self.nodesIndex[e.sourceID])throw Error("Edge's source \""+e.sourceID+'" does not exist yet.');
            if(!self.nodesIndex[e.targetID])throw Error("Edge's target \""+
                e.targetID+'" does not exist yet.');
            self.edges.push(e);
            return self
        }
