...
public class Region {
    ...
    public Region(ANode[] nodes) {
        this.nodes = new ArrayList<ANode>();
        this.nodes.addAll(Arrays.asList(nodes));
        updateMassAndGeometry();
    }

    public Region(ArrayList<ANode> nodes) {
    ...
    public synchronized void buildSubRegions() {
        if (nodes.size() > 1) {
            ArrayList<ANode> leftNodes = new ArrayList<ANode>();
            ArrayList<ANode> rightNodes = new ArrayList<ANode>();
            for (ANode n : nodes) {
                ArrayList<ANode> nodesColumn = (n.x() < massCenterX) ? (leftNodes) : (rightNodes);
                nodesColumn.add(n);
            }

            ArrayList<ANode> topleftNodes = new ArrayList<ANode>();
            ArrayList<ANode> bottomleftNodes = new ArrayList<ANode>();
            for (ANode n : leftNodes) {
                ArrayList<ANode> nodesLine = (n.y() < massCenterY) ? (topleftNodes) : (bottomleftNodes);
                nodesLine.add(n);
            }

            ArrayList<ANode> bottomrightNodes = new ArrayList<ANode>();
            ArrayList<ANode> toprightNodes = new ArrayList<ANode>();
            for (ANode n : rightNodes) {
                ArrayList<ANode> nodesLine = (n.y() < massCenterY) ? (toprightNodes) : (bottomrightNodes);
                nodesLine.add(n);
            }
            ...
            if (topleftNodes.size() > 0) {
                if (topleftNodes.size() < nodes.size()) {
                    ...
                    for (ANode n : topleftNodes) {
                        ArrayList<ANode> oneNodeList = new ArrayList<ANode>();
                        oneNodeList.add(n);
    ...
    public void applyForce(ANode n, RepulsionForce Force, double theta) {
        if (nodes.size() < 2) {
            ANode regionNode = nodes.get(0);
