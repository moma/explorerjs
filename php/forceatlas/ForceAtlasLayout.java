...
public class ForceAtlasLayout {
    ...
    public static ForceVectorNodeLayoutData[] nodesFA;
    ...
    public void initAlgo() throws FileNotFoundException, IOException {
        resetPropertiesValues();
        legraphe = new ExtractData(); //get JSON data
        nodesFA = new ForceVectorNodeLayoutData[legraphe.getNds().size()];
...
//        Edge[] edges = graph.getEdges().toArray();
        ...
        if (isAdjustSizes()) {
            for (Node n1 : nodes) {
                for (Node n2 : nodes) {
                    if (n1 != n2) {
                        ForceVectorUtils.fcBiRepulsor_noCollide(n1, n2, getRepulsionStrength() * (1 + n1.getDegree()) * (1 + n2.getDegree()));
                    }
                }
            }
        } else {
            for (Node n1 : nodes) {
                for (Node n2 : nodes) {
                    if (n1 != n2) {
                        ForceVectorUtils.fcBiRepulsor(n1, n2, getRepulsionStrength() * (1 + n1.getDegree()) * (1 + n2.getDegree()));
                    }
                }
            }
        }
        ...
        if (isAdjustSizes()) {
            if (isOutboundAttractionDistribution()) {
                for (Edge e : edges) {
                    ...
                    bonus *= e.getWeight();
                    ForceVectorUtils.fcBiAttractor_noCollide(nf, nt, bonus * getAttractionStrength() / (1 + nf.getDegree()));
                }
            } else {
                for (Edge e : edges) {
                    Node nf = e.getSource();
            ...
            if (isOutboundAttractionDistribution()) {
                for (Edge e : edges) {
                    ...
                    double bonus = (nf.isFixed() || nt.isFixed()) ? (100) : (1);
                    bonus *= e.getWeight();
                    ForceVectorUtils.fcBiAttractor(nf, nt, bonus * getAttractionStrength() / (1 + nf.getDegree()));
                }
            } else {
                for (Edge e : edges) {
                    Node nf = e.getSource();
